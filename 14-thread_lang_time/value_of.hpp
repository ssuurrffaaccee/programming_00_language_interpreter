...
SPtr<Value> value_of_cont(const SPtr<Expression> &exp, SPtr<Environment> &env,
  ...
  if (exp->type_ == ExpressionType::SET_EXP) {
    ...
    auto set_cont = make_set_cont(set_exp->var_name_, env, cont);
    return value_of_cont(set_exp->exp_, env, set_cont);
  }
  ...
  if (exp->type_ == ExpressionType::MUTEX_EXP) {
    ...
    return apply_cont(cont, to_value(mutex));
  }
  if (exp->type_ == ExpressionType::WAIT_EXP) {
    const WaitExp *wait_exp = get<WaitExp>(exp);
    auto wait_cont = make_wait_cont(wait_exp->mutex_name_, env, cont);
    return apply_cont(wait_cont, to_value(55));
  }
  if (exp->type_ == ExpressionType::SIGNAL_EXP) {
    ...
    if (mutex.status_ == Mutex::MutextStatus::LOCKED) {
      ...
      return apply_cont(cont, to_value(53));
    }
...
SPtr<Value> apply_cont(const SPtr<Continuation> &cont,
                       const SPtr<Value> &value) {
  if (ThreadScheduler::get().is_time_expired()) {
    ThreadScheduler::get().place_on_ready_queue(
        [=]() { return apply_cont(cont, value); });
    auto next_thread = ThreadScheduler::get().get_next_thread();
    return next_thread.operator()();
  }
  ThreadScheduler::get().decrement_timer();
  ...
  if (cont->type_ == ContinuationType::SET_CONT) {
    ...
    return apply_cont(set_cont->cont_, to_value(44));
  }
  ...
  if (cont->type_ == ContinuationType::WAIT_CONT) {
    ...
    Mutex mutex = value_to<MutexValue>(value);
    if (mutex.status_ == Mutex::MutextStatus::LOCKED) {
      ThreadScheduler::get().wait(wait_cont->mutex_name_, [=]() {
        return apply_cont(cont, to_value(55));
      });
      auto next_thread = ThreadScheduler::get().get_next_thread();
      return next_thread.operator()();
    }
    if (mutex.status_ == Mutex::MutextStatus::UNLOCKED) {
      ...
      return apply_cont(wait_cont->cont_, to_value(52));
    }
    CHECK_WITH_INFO(false, "got unkown mutex status!!!")
  }

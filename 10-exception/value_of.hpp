...
SPtr<Value> value_of_cont(const SPtr<Expression> &exp, SPtr<Environment> &env,
  ...
  if (exp->type_ == ExpressionType::TRY_EXP) {
    ...
    auto try_cont =
        make_try_cont(try_exp->exception_var_, try_exp->handler_, env, cont);
    return value_of_cont(try_exp->exp_, env, try_cont);
  }
  if (exp->type_ == ExpressionType::RAISE_EXP) {
    const RaiseExp *raise_exp = get<RaiseExp>(exp);
    auto raise_cont = make_raise1_cont(cont);
    return value_of_cont(raise_exp->exp_, env, raise_cont);
  }
...
SPtr<Value> apply_cont(const SPtr<Continuation> &cont,
  ...
  if (cont->type_ == ContinuationType::OPRAND_CONT) {
    ...
    if (proc_value->type_ == ValueType::PROC) {
      ...
      return value_of_cont(proc.body_, new_env, operand_cont->cont_);
    }
    if (proc_value->type_ == ValueType::REC_PROC) {
      ...
      return value_of_cont(proc.proc_body_, new_env_1, operand_cont->cont_);
    }
  }
  ...
  if (cont->type_ == ContinuationType::TRY_CONT) {
    ...
    return apply_cont(try_cont->cont_, value);
  }
  if (cont->type_ == ContinuationType::RAISE1_CONT) {
    Raise1Cont *raise_cont = get<Raise1Cont>(cont);
    auto cont_ptr = raise_cont->cont_.get();
    while (true) {
      CHECK(cont_ptr != nullptr)
      if (cont_ptr->type_ == ContinuationType::END_CONT) {
        CHECK_WITH_INFO(false, "got unhandled exception:" + to_string(value))
        return nullptr;
      }
      if (cont_ptr->type_ == ContinuationType::TRY_CONT) {
        TryCont *try_cont = (TryCont *)cont_ptr;
        auto new_env = try_cont->the_time_env_->extend(try_cont->var_, value);
        return value_of_cont(try_cont->handler_, new_env, try_cont->cont_);
      }

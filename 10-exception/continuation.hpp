...
struct TryCont : public Continuation {
  TryCont() { type_ = ContinuationType::TRY_CONT; }
  std::string var_;
  SPtr<Expression> handler_;
  SPtr<Environment> the_time_env_;
};
SPtr<Continuation> make_try_cont(const std::string &var,
                                 const SPtr<Expression> &handler,
                                 const SPtr<Environment> &the_time_env,
                                 const SPtr<Continuation> &cont) {
  TryCont *this_cont = new TryCont{};
  this_cont->var_ = var;
  this_cont->handler_ = handler;
  this_cont->the_time_env_ = the_time_env;
  this_cont->cont_ = cont;
  return SPtr<Continuation>{(Continuation *)this_cont};
};
struct Raise1Cont : public Continuation {
  Raise1Cont() { type_ = ContinuationType::RAISE1_CONT; }
};
SPtr<Continuation> make_raise1_cont(const SPtr<Continuation> &cont) {
  Raise1Cont *this_cont = new Raise1Cont{};
  this_cont->cont_ = cont;
  return SPtr<Continuation>{(Continuation *)this_cont};
};
...
#define PRINT_OUT_CONT(CONT_ENUM_NAME)                                             \
  ...
  while (cont_ptr != nullptr) {
    ...
    PRINT_OUT_CONT(TRY_CONT)
    PRINT_OUT_CONT(RAISE1_CONT)

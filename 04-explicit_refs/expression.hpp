...
struct NewRefExp : public Expression {
  NewRefExp() { type_ = ExpressionType::NEWREF_EXP; }
  SPtr<Expression> exp_;
};
SPtr<Expression> new_ref(const SPtr<Expression> &e) {
  NewRefExp *exp = new NewRefExp{};
  exp->exp_ = e;
  return SPtr<Expression>{(Expression *)exp};
}
struct DerefExp : public Expression {
  DerefExp() { type_ = ExpressionType::DEREF_EXP; }
  SPtr<Expression> exp_;
};
SPtr<Expression> deref(const SPtr<Expression> &e) {
  DerefExp *exp = new DerefExp{};
  exp->exp_ = e;
  return SPtr<Expression>{(Expression *)exp};
}
struct SetRefExp : public Expression {
  SetRefExp() { type_ = ExpressionType::SET_REF_EXP; }
  SPtr<Expression> ref_exp_;
  SPtr<Expression> value_exp_;
};
SPtr<Expression> set_ref(const SPtr<Expression> &ref_exp,
                         const SPtr<Expression> &value_exp) {
  SetRefExp *exp = new SetRefExp{};
  exp->ref_exp_ = ref_exp;

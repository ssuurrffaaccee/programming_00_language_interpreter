...
SPtr<Environment> evaluate_module(const SPtr<Expression> &exp,
...
SPtr<Environment> evaluate_module(const SPtr<Expression> &exp,
                                  SPtr<Environment> &env) {
  CHECK(exp->type_ == ExpressionType::MODULE_EXP)
  const ModuleExp *module_exp = get<ModuleExp>(exp);
  auto model_name = module_exp->name_;
  for (auto &[name, impl_exp] : module_exp->body_) {
    auto value = value_of(impl_exp, env);
    env = env->extend(name, value);
  }
  for (auto &[name, _] : module_exp->body_) {
    auto value = env->apply(name);
    env = env->extend(model_name, name, value);
  }
  return env;
}
#pragma once
#include <string>
#include <vector>

#include "check.hpp"
#include "sptr.hpp"
#include "type.hpp"
// Expression
enum class ExpressionType {
  UNKOWN_EXP,
  CONST_EXP,
  VAR_EXP,
  DIFF_EXP,
  ADD_EXP,
  IS_ZERO_EXP,
  IF_EXP,
  LET_EXP,
  CALL_EXP,
  PROC_EXP,
  LETREC_EXP,
  BEGIN_EXP,
  PRINT_EXP,
  MODULE_EXP,
  IMPORT_EXP,
};
struct Expression {
  ExpressionType type_;
  virtual ~Expression() {}
};

struct ConstExp : public Expression {
  ConstExp() { type_ = ExpressionType::CONST_EXP; }
  int num_;
};
SPtr<Expression> const_(int v) {
  ConstExp *exp = new ConstExp{};
  exp->num_ = v;
  return SPtr<Expression>{(Expression *)exp};
}

struct VarExp : public Expression {
  VarExp() { type_ = ExpressionType::VAR_EXP; }
  std::string var_;
};
SPtr<Expression> var(const std::string &name) {
  VarExp *exp = new VarExp{};
  exp->var_ = name;
  return SPtr<Expression>{(Expression *)exp};
}

struct DiffExp : public Expression {
  DiffExp() { type_ = ExpressionType::DIFF_EXP; }
  SPtr<Expression> left_exp_;
  SPtr<Expression> right_exp_;
};
SPtr<Expression> diff(const SPtr<Expression> &l, const SPtr<Expression> &r) {
  DiffExp *exp = new DiffExp{};
  exp->left_exp_ = l;
  exp->right_exp_ = r;
  return SPtr<Expression>{(Expression *)exp};
}

struct AddExp : public Expression {
  AddExp() { type_ = ExpressionType::ADD_EXP; }
  SPtr<Expression> left_exp_;
  SPtr<Expression> right_exp_;
};
SPtr<Expression> add(const SPtr<Expression> &l, const SPtr<Expression> &r) {
  AddExp *exp = new AddExp{};
  exp->left_exp_ = l;
  exp->right_exp_ = r;
  return SPtr<Expression>{(Expression *)exp};
}

struct IsZeroExp : public Expression {
  IsZeroExp() { type_ = ExpressionType::IS_ZERO_EXP; }
  SPtr<Expression> exp_;
};
SPtr<Expression> is_zero(const SPtr<Expression> &e) {
  IsZeroExp *exp = new IsZeroExp{};
  exp->exp_ = e;
  return SPtr<Expression>{(Expression *)exp};
}

struct IfExp : public Expression {
  IfExp() { type_ = ExpressionType::IF_EXP; }
  SPtr<Expression> condition_exp_;
  SPtr<Expression> true_exp_;
  SPtr<Expression> false_exp_;
};
SPtr<Expression> if_(const SPtr<Expression> &condition_exp,
                     const SPtr<Expression> &true_exp,
                     const SPtr<Expression> &false_exp) {
  IfExp *exp = new IfExp{};
  exp->condition_exp_ = condition_exp;
  exp->true_exp_ = true_exp;
  exp->false_exp_ = false_exp;
  return SPtr<Expression>{(Expression *)exp};
}
struct LetExp : public Expression {
  LetExp() { type_ = ExpressionType::LET_EXP; }
  std::string var_;
  SPtr<Expression> exp_;
  SPtr<Expression> body_;
};
SPtr<Expression> let(const std::string &var, const SPtr<Expression> &exp,
                     const SPtr<Expression> &body) {
  LetExp *let_exp = new LetExp{};
  let_exp->var_ = var;
  let_exp->exp_ = exp;
  let_exp->body_ = body;
  return SPtr<Expression>{(Expression *)let_exp};
}

struct ProcExp : public Expression {
  ProcExp() { type_ = ExpressionType::PROC_EXP; }
  std::string parameter_;
  SPtr<Type> parameter_type_;
  SPtr<Expression> body_;
};

SPtr<Expression> proc(const std::string &parameter,
                      const SPtr<Type> paramter_type,
                      const SPtr<Expression> &body) {
  ProcExp *exp = new ProcExp{};
  exp->parameter_ = parameter;
  exp->parameter_type_ = paramter_type;
  exp->body_ = body;
  return SPtr<Expression>{(Expression *)exp};
}

struct CallExp : public Expression {
  CallExp() { type_ = ExpressionType::CALL_EXP; }
  SPtr<Expression> proc_generator_;
  SPtr<Expression> arg_;
};
SPtr<Expression> call(const SPtr<Expression> &proc_generator,
                      const SPtr<Expression> &arg) {
  CallExp *exp = new CallExp{};
  exp->proc_generator_ = proc_generator;
  exp->arg_ = arg;
  return SPtr<Expression>{(Expression *)exp};
}

struct LetRecExp : public Expression {
  LetRecExp() { type_ = ExpressionType::LETREC_EXP; }
  std::string proc_name_;
  std::string parameter_;
  SPtr<Type> parameter_type_;
  SPtr<Type> result_type_;
  SPtr<Expression> proc_body_;
  SPtr<Expression> apply_body_;
};

SPtr<Expression> let_rec(const std::string &proc_name,
                         const std::string &parameter,
                         SPtr<Type> parameter_type, SPtr<Type> result_type,
                         const SPtr<Expression> &proc_body,
                         const SPtr<Expression> &apply_body) {
  LetRecExp *exp = new LetRecExp{};
  exp->proc_name_ = proc_name;
  exp->parameter_ = parameter;
  exp->parameter_type_ = parameter_type;
  exp->result_type_ = result_type;
  exp->proc_body_ = proc_body;
  exp->apply_body_ = apply_body;
  return SPtr<Expression>{(Expression *)exp};
}
struct BeginExp : public Expression {
  BeginExp() { type_ = ExpressionType::BEGIN_EXP; }
  std::vector<SPtr<Expression>> exps_;
};
template <typename... Args>
SPtr<Expression> begin(const Args &...args) {
  BeginExp *exp = new BeginExp{};
  (exp->exps_.push_back(args), ...);
  return SPtr<Expression>{(Expression *)exp};
}
struct PrintExp : public Expression {
  PrintExp() { type_ = ExpressionType::PRINT_EXP; }
  SPtr<Expression> exp_;
};
SPtr<Expression> print(const SPtr<Expression> &e) {
  PrintExp *exp = new PrintExp{};
  exp->exp_ = e;
  return SPtr<Expression>{(Expression *)exp};
}
namespace module_helper {
struct Declaration {
  Declaration(const std::string &name, const SPtr<Type> &type)
      : name_{name}, type_{type} {}
  std::string name_;
  SPtr<Type> type_;
};
struct Interface {
  std::unordered_map<std::string, SPtr<Type>> decls_;
};
struct Implemntation {
  Implemntation(const std::string &name, const SPtr<Expression> &exp)
      : name_{name}, exp_{exp} {}
  std::string name_;
  SPtr<Expression> exp_;
};
struct Body {
  std::unordered_map<std::string, SPtr<Expression>> impls_;
};
}  // namespace module_helper
SPtr<module_helper::Interface> interface(
    const std::vector<SPtr<module_helper::Declaration>> &decls) {
  auto interface = std::make_shared<module_helper::Interface>();
  for (auto &decl : decls) {
    interface->decls_[decl->name_] = decl->type_;
  }
  return interface;
}
SPtr<module_helper::Declaration> decl(const std::string &name,
                                      const SPtr<Type> &type) {
  return std::make_shared<module_helper::Declaration>(name, type);
}
SPtr<module_helper::Body> body(
    const std::vector<SPtr<module_helper::Implemntation>> &impls) {
  auto body = std::make_shared<module_helper::Body>();
  for (auto &impl : impls) {
    body->impls_[impl->name_] = impl->exp_;
  }
  return body;
}
SPtr<module_helper::Implemntation> impl(const std::string &name,
                                        const SPtr<Expression> &exp) {
  return std::make_shared<module_helper::Implemntation>(name, exp);
}
struct ModuleExp : public Expression {
  ModuleExp() { type_ = ExpressionType::MODULE_EXP; }
  std::string name_;
  std::unordered_map<std::string, SPtr<Type>> interface_;
  std::unordered_map<std::string, SPtr<Expression>> body_;
};
SPtr<Expression> module_(const std::string &name,
                         const SPtr<module_helper::Interface> interface,
                         const SPtr<module_helper::Body> &body) {
  ModuleExp *exp = new ModuleExp{};
  exp->name_ = name;
  for (auto &[name, type] : interface->decls_) {
    auto iter = exp->interface_.find(name);
    CHECK(iter == exp->interface_.end())
    exp->interface_[name] = type;
  }
  for (auto &[name, impl_exp] : body->impls_) {
    auto iter = exp->body_.find(name);
    CHECK(iter == exp->body_.end())
    exp->body_[name] = impl_exp;
  }
  return SPtr<Expression>{(Expression *)exp};
}

struct ImportExp : public Expression {
  ImportExp() { type_ = ExpressionType::IMPORT_EXP; }
  std::string model_name_;
  std::string component_name_;
};
SPtr<Expression> import_(const std::string &model_name,
                         const std::string &component_name) {
  ImportExp *exp = new ImportExp{};
  exp->model_name_ = model_name;
  exp->component_name_ = component_name;
  return SPtr<Expression>{(Expression *)exp};
}

template <typename T>
T *get(const SPtr<Expression> &exp) {
  return (T *)exp.get();
}
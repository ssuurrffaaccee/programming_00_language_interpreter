...
struct AssignExp : public Expression {
  AssignExp() { type_ = ExpressionType::ASSIGN_EXP; }
  std::string name_;
  SPtr<Expression> value_exp_;
};
SPtr<Expression> assign(const std::string &name,
                        const SPtr<Expression> &value_exp) {
  AssignExp *exp = new AssignExp{};
  exp->name_ = name;
  exp->value_exp_ = value_exp;
  return SPtr<Expression>{(Expression *)exp};
}
...
struct MakePairExp : public Expression {
  MakePairExp() { type_ = ExpressionType::MAKE_PAIR_EXP; }
  SPtr<Expression> left_exp_;
  SPtr<Expression> right_exp_;
};
SPtr<Expression> mk_pair(const SPtr<Expression> &left_exp,
                         const SPtr<Expression> &right_exp) {
  MakePairExp *exp = new MakePairExp{};
  exp->left_exp_ = left_exp;
  exp->right_exp_ = right_exp;
  return SPtr<Expression>{(Expression *)exp};
}
struct LeftExp : public Expression {
  LeftExp() { type_ = ExpressionType::LEFT_EXP; }
  SPtr<Expression> pair_exp_;
};
SPtr<Expression> left(const SPtr<Expression> &pair_exp) {
  LeftExp *exp = new LeftExp{};
  exp->pair_exp_ = pair_exp;
  return SPtr<Expression>{(Expression *)exp};
}
struct RightExp : public Expression {
  RightExp() { type_ = ExpressionType::RIGHT_EXP; }
  SPtr<Expression> pair_exp_;
};
SPtr<Expression> right(const SPtr<Expression> &pair_exp) {
  RightExp *exp = new RightExp{};
  exp->pair_exp_ = pair_exp;
  return SPtr<Expression>{(Expression *)exp};
}
struct SetLeftExp : public Expression {
  SetLeftExp() { type_ = ExpressionType::SET_LEFT_EXP; }
  SPtr<Expression> pair_exp_;
  SPtr<Expression> value_exp_;
};
SPtr<Expression> set_left(const SPtr<Expression> &pair_exp,
                          const SPtr<Expression> &value_exp) {
  SetLeftExp *exp = new SetLeftExp{};
  exp->pair_exp_ = pair_exp;
  exp->value_exp_ = value_exp;
  return SPtr<Expression>{(Expression *)exp};
}
struct SetRightExp : public Expression {
  SetRightExp() { type_ = ExpressionType::SET_RIGHT_EXP; }
  SPtr<Expression> pair_exp_;
  SPtr<Expression> value_exp_;
};
SPtr<Expression> set_right(const SPtr<Expression> &pair_exp,
                           const SPtr<Expression> &value_exp) {
  SetRightExp *exp = new SetRightExp{};
  exp->pair_exp_ = pair_exp;
  exp->value_exp_ = value_exp;
  return SPtr<Expression>{(Expression *)exp};
}

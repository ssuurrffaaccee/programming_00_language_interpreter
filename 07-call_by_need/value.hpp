...
struct Pair {
  Ref left_value_ref_;
  Ref right_value_ref_;
};
std::string to_string(const Pair &v) {
  return std::string("PAIR:(") + to_string(v.left_value_ref_) + "," +
         to_string(v.right_value_ref_) + ")";
}
DEFINE_VALUE(PairValue, PAIR, Pair)
...
std::string to_string(const Lazy &v) {
  return std::string("Lazy");
}
DEFINE_VALUE(LazyValue, LAZY, Lazy)